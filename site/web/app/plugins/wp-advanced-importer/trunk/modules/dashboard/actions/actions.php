...
class DashboardActions extends SkinnyActions {
    ...
    $mons[]= date("M", strtotime( $today." -$i months"));
                    ...
                    {
   $lid = $wpdb->get_results("select inserted from smackxml_line_log where imported_type = '{$imp_type}' and imported_on >= DATE_SUB(NOW(),INTERVAL 1 YEAR)");
                              foreach($lid as $ll) {
                                     $change = $ll->inserted;
                                      $today = date("Y-m-d H:i:s"); 
    ...
    $years[]= date("Y", strtotime( $today." -$i months"));
        ...
        $tot = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][0]}' and imported_type = '{$imp_type}' and year = {$random['y'][0]} ");
        foreach($tot as $tt) { $total .= (int)$tt ->ins.","; } $chart1 .= $total ;
        
        $tot1 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][1]}'  and imported_type = '{$imp_type}' and year = {$random['y'][1]}");
        foreach($tot1 as $tt1) { $total1 .= (int)$tt1 ->ins.","; } $chart1 .= $total1  ;
        
        $tot2 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][2]}' and imported_type = '{$imp_type}' and year = {$random['y'][2]} ");
        foreach($tot2 as $tt2) { $total2 .= (int)$tt2->ins.","; } $chart1 .= $total2 ;
        
        $tot3 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][3]}' and imported_type = '{$imp_type}' and year = {$random['y'][3]}");
        foreach($tot3 as $tt3) { $total3 .= (int)$tt3 ->ins.","; } $chart1 .= $total3 ;
                                
        $tot4 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][4]}' and imported_type = '{$imp_type}' and year = {$random['y'][4]} ");
        foreach($tot4 as $tt4) { $total4 .= (int)$tt4 ->ins.","; } $chart1 .= $total4 ;
        
        $tot5 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][5]}'  and imported_type = '{$imp_type}' and year = {$random['y'][5]}");
        foreach($tot5 as $tt5) { $total5 .= (int)$tt5 ->ins.","; } $chart1 .= $total5 ;
        
        $tot6 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][6]}' and imported_type = '{$imp_type}' and year = {$random['y'][6]} ");
        foreach($tot6 as $tt6) { $total6 .= (int)$tt6 ->ins.","; } $chart1 .= $total6 ;
        
        $tot7 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][7]}' and imported_type = '{$imp_type}' and year = {$random['y'][7]} ");
        foreach($tot7 as $tt7) { $total7 .= (int)$tt7 ->ins.","; } $chart1 .= $total7 ;
                
        $tot8 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][8]}'  and imported_type = '{$imp_type}' and year = {$random['y'][8]}");
        foreach($tot8 as $tt8) { $total8 .= (int)$tt8 ->ins.","; } $chart1 .= $total8 ;
        
        $tot9 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][9]}' and imported_type = '{$imp_type}' and year = {$random['y'][9]} ");
        foreach($tot9 as $tt9) { $total9 .= (int)$tt9 ->ins.","; } $chart1 .= $total9 ;
        
        $tot10 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][10]}' and imported_type = '{$imp_type}' and year = {$random['y'][10]} ");
        foreach($tot10 as $tt10) { $total10 .= (int)$tt10 ->ins.","; } $chart1 .= $total10 ;
        
        $tot11 = $wpdb->get_results("select sum(inserted) as ins from smackxml_line_log where month ='{$random['m'][11]}' and imported_type = '{$imp_type}' and year ={$random['y'][11]} ");
